...
function add_coins(loot, amount) {
  ...
  loot.addItem(Item.of('lightmanscurrency:coin_copper', amount_copper));
  loot.addItem(Item.of('lightmanscurrency:coin_iron', amount_iron));
  loot.addItem(Item.of('lightmanscurrency:coin_gold', amount_gold));
  loot.addItem(Item.of('lightmanscurrency:coin_emerald', amount_emerald));
  loot.addItem(Item.of('lightmanscurrency:coin_diamond', amount_diamond));
  loot.addItem(Item.of('lightmanscurrency:coin_netherite', amount_netherite));
...
LootJS.modifiers((event) => {
  ...
  event
    .addTableModifier(COIN_LOOT_LOW)
    .randomChance(0.3)
    .customAction((context, loot) => {
      let p = context.getPosition();
      ...
      amount *= multiplier;
      add_coins(loot, amount); // 1-5 near spawn, 2-10 far away
  });
  ...
  event
    .addTableModifier(COIN_LOOT_MEDIUM)
    .randomChance(0.6)
    .customAction((context, loot) => {
      let p = context.getPosition();
      let distance = Math.min(Math.sqrt(p.x * p.x + p.z * p.z), 10000);
      ...
      amount *= multiplier;
      add_coins(loot, amount); // 5-10 near spawn, 10-20 far away
  });
  ...
  event
    .addTableModifier(COIN_LOOT_TREASURE)
    .customAction((context, loot) => {
      let p = context.getPosition();
      ...
      add_coins(loot, amount); // 10-50 near spawn, 20-100 far away + bonus
  });
